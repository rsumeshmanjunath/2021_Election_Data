...
from scrapy_tutorial.items import CandidateItem

class CandidateItemSpider(scrapy.Spider):
   name = "candidates_item"
   ...
   def parse(self, response):
       ...
       for candidate in candidates:
           ...
           if((temp[2].strip() == 'Tamil Nadu') and (status.strip() == 'Accepted')):
               ...
               loader = ItemLoader(item = CandidateItem(), selector=candidate)
